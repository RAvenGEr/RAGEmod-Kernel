		...
		unsigned int relation)
{
	...
	if (s5pv310_max_armclk == ARMCLOCK_1400MHZ) {
		...
		/* change L3 -> L1 */
		if ((index == L1) && (old_index > L3)) {
			printk(KERN_ERR "index= %d, old_index= %d\n", index, old_index);
			index = L3;
		}
#else
		/* change L3 -> L2 and change L2 -> L1 */
		if (index == L1) {
			printk(KERN_ERR "index= %d, old_index= %d\n", index, old_index);
			if (old_index > L2)
				index = L2;
...
static int s5pv310_asv_table_update(void)
{
	...
	for (i = 0; i < last_level; i++) {
		...
		/* Maximum Voltage */
/*		if (s5pv310_volt_table[i].arm_volt > 1400000)
			s5pv310_volt_table[i].arm_volt = 1400000;
*/
